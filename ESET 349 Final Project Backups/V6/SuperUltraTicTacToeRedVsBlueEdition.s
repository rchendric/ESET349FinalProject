...
WinnerWinner function ;flashes winning player's indicator LED
			;PlayerPortOffset passed by r10
			;PlayerPin passed by r11
			;r0 holds the base address
			;r1 holds offset
			;r5 passes player index
			;r2 holds current pin mask
			;r3 holds register contents
			;r4 holds delay function
			STMFD sp!, {lr}
			LDR r0, =0x40004C00 ;this loads base address
			LDRB r1, [r10, r5] ;loads offset at index pos from port offset array
			ADD r0, r0, r1 ; adds offset to base address
			LDRB r2, [r11, r5] ;loads pin at index from pin array
			LDRB r3, [r0, #0x02] ;Loads current output from port
WinLoop		EOR r3, r2, r3 ;sets r2 pin output to high(1)
			STRB r3, [r0, #0x02]  ;Stores updated output back to port			
			MOV r4, #0x20000
WinDelay	SUBS r4, #0x01
			CMP r4, #0x00
			BNE WinDelay
			B WinLoop
...
SwitchInitLoop	
			...
			LDRB r3, [r0, #0x02] ;Loads current pin Pull up/down from port
			AND r3, r2, r3 ;sets current pin to pull down without altering others
			STRB r3, [r0, #0x02] ;Stores updated pull up/down to port
